...
| Thứ tự lệch | `⚠ thứ tự shot trong file (1,2,4,3) khác thứ tự thời gian` | viết shot không theo dòng lời; **hay nhất là `trong` bị append xuống cuối chương** thay vì chèn ngay trước shot nó dẫn vào | sắp lại theo thứ tự `vo`; `trong` chèn đúng vị trí thời gian trong mảng `shots`. Renderer sắp theo mốc nên vẫn chạy, nhưng `--moc` đọc rối và người sửa sau đặt `tai` nhầm |
...
| Shot quá ngắn | `shot 7 "Xoá": chỉ 0.18s (< 0.25s)` | hai `say` quá sát trong lời (dưới 8 khung), ví dụ *"Chặt."* → *"Rất chặt."*. Gộp vào shot trước (chữ `the` hai dòng `"CHẶT.\nRẤT CHẶT."` `tung-tu`), hoặc bỏ shot sau. Luôn chạy `--moc` với board nháp trước khi đặt `trong`/`act` (SKILL bước 5) |
...
| Nhịp trắng đặt lệch | `⚠ dai=0.5s nhưng shot sau tới sau 0.12s — bị cắt` hoặc trắng rơi giữa câu đang nói | `tai` của `trong` phải = mốc shot kế **trừ** `dai`. Đọc mốc bằng `kiem-tra-v2 --moc`, ví dụ shot kế 15.74 s → `{"tai": 15.23, "dai": 0.5}` **chèn ngay trước shot 15.74 trong mảng** (không append cuối). Sửa `vo` xong mốc trôi: đặt lại |
...
| Chữ `the` dài tràn qua nhân vật | khối `the` rộng 0.9 W; dòng > 14 ký tự ở `trai`/`phai` chạm nửa khung bên kia; `phong` phóng 1.6× | ngắt dòng bằng `"\n"`: kèm nhân vật ≤ 14 ký tự/dòng, `phong` ≤ 12, `giua` ≤ 20 (dao-dien §7) |
| Chữ đè lên phụ đề ở đáy | `vi_tri: duoi` (tâm y 0.86) trùng vùng phụ đề tự động (y 0.90–0.97) | shot có lời → dùng `tren` hoặc `giua`; `duoi` chỉ ở shot không có lời |
| Thầy / mẹ / đầu trắng mất đỉnh đầu ở cỡ `can`, chỉ còn nửa mặt ở `sat` | khung người lớn 3.6 đầu (≈ 2.95 DAU cỡ cảnh sau khi renderer thu 0.82), chân ở `chanY` nên đầu vọt mép trên | `co: 0.85–0.9` ở `can`, `co: 0.8` ở `sat` cho `thay`/`me`/`trang` (bảng dao-dien §2) |
| Bối cảnh vẽ hai cái bảng / hai cái đồng hồ | dùng `boi_canh` rồi khai lại prop đã nằm trong nó | xoá prop trùng; danh sách prop của từng bối cảnh: dao-dien §6 hoặc `thu-vien-v2.json` mục `boi_canh` |
| Lòi một mảng đồ to sau lưng ở shot cận | `boi_canh` ở cỡ `can`/`sat` (bộ `trung` phóng theo DAU) | bỏ `boi_canh` ở shot `can`/`sat`; bối cảnh chỉ cho `rong`/`trung` |
...
| Prop bé tí hoặc khổng lồ | `co` nhân theo cỡ cảnh (`dau/330`): cùng `co: 1` ở `rong` = 0.6×, ở `can` = 1.7×, ở `sat` = 2.7× so với `trung` | công thức `px màn = px khai × co × DAU/330`; muốn cao ~65% khung: `co ≈ 700 / (H khai × DAU/330)` — `but` (~150 px) ở `sat` co 1.6–1.8, `dien-thoai` (260 px) ở `sat` co 0.9, đồ 560 px ở `sat` co 0.45 (bảng dao-dien §2). Prop tự vẽ: `xem-prop.mjs --co-canh sat --thu 0.25` |
| Hai cái bút: một trên tay, một cạnh tay | vừa `cam: "but"` vừa `prop` `but` đặt cạnh tay | `cam` đã vẽ vật trên tay — bỏ prop trùng |
...
| Triệu chứng | Sửa |
|---|---|
| `npx tsc` đỏ ở bước `dang-ky` | `board.json` phải là JSON hợp lệ (không comment, không dấu phẩy cuối) và đúng kiểu (`x` số, `act` mảng, `chu` mảng). Không sửa `src/projects/du-an.generated.ts` tay — `dang-ky.mjs` sinh lại |
| Composition `V2-<ten>` không thấy | `dang-ky.mjs` bỏ qua dự án thiếu file: cần đủ `board.json` + `data/<ten>.{generated,voice,mouth}.json` và kịch bản có `project`. Chạy `node pipeline/dang-ky.mjs` xem lý do in ra. Không sửa `src/Root.tsx` |
| Video ngắn hơn audio, cụt cuối / ghép đôi đoạn | không xảy ra với `tao-video.mjs` (render theo đoạn có khoá, `cham-diem` kiểm thời lượng khớp audio). Nếu render tay bằng `render-segments.sh`, xoá `out/.seg-V2-<ten>-*` rồi chạy lại |
...
| Still đen / rỗng | frame rơi vào intro (< 60) hoặc vượt tổng. Đừng tính frame tay: `node pipeline/xem-shot.mjs <ten> <chuong> "<say>"` hoặc `"#<i>"` render đúng khung shot |
| `xem-shot` báo `không tìm thấy shot khớp` | `say` phải khớp nguyên văn (đã bỏ dấu câu) hoặc là chuỗi con của `say` trong board; hoặc dùng `#<i>` với `<i>` là vị trí trong mảng `shots` đếm từ 0 |
